    ...
    *handle.device_name.write().unwrap() = device_name(&device);
    ...
    let stream = device.build_output_stream(
        ...
        move |data: &mut [f32], _| {
            ...
            // Advance the reported position from what's actually draining
            // out right now — only `filled` real samples, never the
            // zero-padded underrun tail below (there's no decoded audio
            // behind that padding to attribute to any seq).
            ...
            if let Some(seq) = current {
                ...
                if producer.vacant_len() >= segment_samples {
                    ...
                    if let Some(path) = path {
                        ...
                        match decode_to_pcm(&path, sample_rate) {
                            Ok(pcm) => {
                                ...
                                handle.enqueue_pending(seq, samples);
                            ...
                            Err(err) => {
                                ...
                                *handle.stall_reason.write().unwrap() =
                                ...
                                // No audio reached the ring — nothing for the
                                // output callback to drain, so this reports
                                // as passed the moment the callback next runs.
                    ...
                    if advance {
                        ...
                        // `drain_pending`, driven by the output callback
                        // (see `PlayoutHandle::pending`'s doc comment for
                        // why: advancing it the instant a segment is queued
                        // would report a position up to a full ring's worth
                        // ahead of what's actually audible).
                    }
                }
            }
        }
