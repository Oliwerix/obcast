...
pub fn plan_uploads(input: &SchedulerInput) -> Vec<UploadAction> {
    ...
    // forward; fill any hole until we've secured `target_ms`. With an active
    // head to defend, always the low rung — the no-dropout guarantee must
    // never depend on the operator's quality pick being achievable. Without
    // one, try `preferred_rung` first (same affordability/availability check
